...
//! ROM path to stdout and exits 0; on cancel it exits 1. Covers stream in on a
//! background thread ("preenchimento progressivo", plan §3.1).
...
fn main() -> Result<()> {
    ...
    loop {
        ...
        frame_no += 1;
        if max_frames.is_some_and(|n| frame_no > n) {
            return Ok(());
        }
